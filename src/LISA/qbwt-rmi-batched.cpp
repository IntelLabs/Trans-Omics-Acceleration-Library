...
//				fprintf(stderr, "%ld %ld -- %ld %ld", next.low, next.high, next_tal.first, next_tal.second);

				//assert((next.low == next_tal.first && next.high == next_tal.second));
...
//						output->smem[td.numSMEMs] = SMEM_out(q.id, q.l, q.r, q.intv.first, q.intv.second);             
		...
		if(next_q >= qs_size || !(fmi_cnt < batch_size)){
			...
			fmi_extend_batched(qbwt, cnt, &fmi_pool[0], td, output, min_seed_len, tal_fmi);	
