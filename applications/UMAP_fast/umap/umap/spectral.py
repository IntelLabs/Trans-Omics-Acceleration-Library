...
def mkl_eigenvalue_d(
    ...
    py_mkl_sparse_destroy.argtypes=[
    sparse_matrix_t
    ]
    ...
    pm[7] = 1                            #0 = relative,  1 = Use absolute stopping critertia. Iteration is stopped if norm(Ax - lambda*x) < 10^(-pm[1])
...
def mkl_eigenvalue_s(  # single-precision version
    spM,
    k0,
    whichE,
    ncv,
...
def spectral_layout(data, graph, dim, random_state, metric="euclidean", metric_kwds={}):
    ...
    # print("Types for eigsh:", type(L), type(k), type(num_lanczos_vectors))
    try:
