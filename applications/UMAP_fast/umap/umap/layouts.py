...
def _optimize_layout_euclidean_single_epoch(
    ...
    for i in numba.prange(epochs_per_sample.shape[0]):                                  # 23029392
        if epoch_of_next_sample[i] <= n:                                                # 6928928

            j = head[i]                                                                 # 4*4    (Size = 4*23029392)
            k = tail[i]                                                                 # 4*4    (Size = 4*23029392)

            current = head_embedding[j]                                                 # 64*2   (Size = 16*990000)
            other = tail_embedding[k]                                                   # 64     (Size = 16*990000)
            ...
            epoch_of_next_sample[i] += epochs_per_sample[i]                            # 4*4 + 4*8 = 48 (if double 96)  (Size = 2*8*23029392)

            n_neg_samples = int(
                (n - epoch_of_next_negative_sample[i]) / epochs_per_negative_sample[i]  # 4*4 + 8*4 = 48 (if double 96) (Size = 2*8*23029392)
            )

            for p in range(n_neg_samples):                                      # 5

                k = tau_rand_int(rng_state) % n_vertices

                other = tail_embedding[k]                                       # 64
...
def optimize_layout_euclidean(
    ...
    for n in range(n_epochs):
        ...
        # if n==0:
        #     print(
        #         head_embedding.dtype,
        #         tail_embedding.dtype,
        #         head.dtype,
        #         tail.dtype,
        #         type(n_vertices),
        #         epochs_per_sample.dtype,
        #         type(a),
        #         type(b),
        #         rng_state.dtype,
        #         type(gamma),
        ...
        #         type(move_other),
        #         type(alpha),
        #         epochs_per_negative_sample.dtype,
        #         epoch_of_next_negative_sample.dtype,
        #         epoch_of_next_sample.dtype,
        #         type(n),
        #         type(densmap_flag),
        #         dens_phi_sum.dtype,
        #         dens_re_sum.dtype,
